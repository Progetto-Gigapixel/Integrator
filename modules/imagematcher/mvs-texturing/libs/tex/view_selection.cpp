 ...
 *
 * This function takes a cost matrix and a graph and sets the labels of the
 * graph according to the result of the MAP-MAP optimization. The first column
 * of the cost matrix is used as the unary term, the first row is ignored.
 *
 * This function does not return anything, but sets the labels of the graph
 * object.
 *
 * @param data_costs A matrix of costs, where data_costs(i, j) is the cost of
 *                   assigning label j to face i.
 * @param graph A UniGraph object, which stores the mesh and its neighborhoods.
...
view_selection(DataCosts const & data_costs, UniGraph * graph, Settings const &) {
    ...
    ctr.use_multilevel = true;//paratemetri??
