...
photometric_outlier_detection(std::vector<FaceProjectionInfo> * infos, Settings const & settings) {
    ...
    double const gauss_rejection_threshold = 60;
...
/**
 * Calculates the projection information for each face of a mesh from
 * multiple texture views, updating the face projection information list.
 * The function uses OpenMP for parallel processing across the texture views.
 *
 * @param mesh The input triangle mesh containing faces and vertices.
 * @param texture_views A vector of TextureView objects representing the
 *        different views of the mesh.
 * @param settings Runtime configuration settings used for processing.
 * @param face_projection_infos A vector to store the calculated projection
 *        information for each face from all views.
 */
...
calculate_face_projection_infos(mve::TriangleMesh::ConstPtr mesh,
    ...
    #pragma omp parallel
    {
        ...
        for (std::uint16_t j = 0; j < static_cast<std::uint16_t>(num_views); ++j) {
            ...
            //le facce sono triangoli, quindi sono 3 vertici, faces size in teoria è un multiplo di 3
            for (std::size_t i = 0; i < faces.size(); i += 3) {
                ...
                //It removes triangles (or polygons) that face away from the camera since they are not visible.
...
/**
 * Postprocess face projection infos.
 *
 * This function removes outliers from the face projection infos and then
 * normalizes the quality values.
 *
 * @param settings The settings of the algorithm.
 * @param face_projection_infos The face projection infos.
 * @param data_costs The data costs for the MRF.
 */
void
...
/**
 * Calculates the data costs for a given mesh and texture views.
 *
 * This function computes the data costs for each face and texture view 
 * combination by first calculating the face projection information and 
 * then postprocessing it to remove outliers and normalize the quality 
 * values. The resulting data costs are stored in the provided DataCosts 
 * structure.
 *
 * @param mesh The triangle mesh for which data costs are calculated.
 * @param texture_views The texture views used for calculating data costs.
 * @param settings The settings that influence the calculation process.
 * @param data_costs The data structure where the resulting data costs 
 *                   will be stored.
 *
 * @throws std::runtime_error if the number of faces or views exceed 
...
calculate_data_costs(mve::TriangleMesh::ConstPtr mesh, std::vector<TextureView> * texture_views,
    ...
    std::size_t const num_views = texture_views->size();
    std::cout << "DEBUG[numero facce = " << num_faces << ", numero viste = " << num_views << "]" << std::endl;
    ...
    std::cout << "DEBUG[calcolo face_projection_infos]" << std::endl;
    calculate_face_projection_infos(mesh, texture_views, settings, &face_projection_infos);
    //std::cout << "DEBUG[face_projection_infos[0][0].quality = " << face_projection_infos[0][0].quality << "]" << std::endl;
