...
/**
 * Generates TextureViews from a folder containing images and corresponding
 * camera files.
 *
 * The camera files must have the same name as the image files but with the
 * extension ".cam" instead of the image file extension.
 *
 * The camera files must contain the intrinsic parameters of the camera.
 * The file should have the following format:
 *   fx fy cx cy
 *   k1 k2 p1 p2
 *
 * The extrinsic parameters are ignored.
 *
 * The intrinsic parameters are used to undistort the images if the distortion
 * parameters are not zero.
 *
 * The function returns a vector of TextureViews with the undistorted images.
 *
 * @param path The path to the folder containing the images and camera files.
 * @param texture_views The vector to store the generated TextureViews.
 * @param tmp_dir The directory to store the undistorted images.
 */
void
    ...
    std::vector<TextureView> * texture_views, std::string const & tmp_dir)
{
    ...
    //     }
    // }
    std::cout << "DEBUG[try to load from dir]" << std::endl;
