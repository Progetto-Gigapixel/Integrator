...
#define MAX_TEXTURE_SIZE (4*16384)

#define PREF_TEXTURE_SIZE (1*16384)

#define MIN_TEXTURE_SIZE (256)
/**
 * Calculates an appropriate size for a texture atlas based on the provided
 * texture patches. The function iteratively reduces the texture atlas size
 * while ensuring that none of the individual texture patches exceed the
 * maximum allowable dimensions. The decision to adjust the size is based
 * on the dimensions and area of the texture patches, as well as the
 * specified padding.
 *
 * @param texture_patches A list of constant shared pointers to TexturePatch
 *        objects, representing the texture patches for which the atlas size
 *        is to be calculated.
 * @return The calculated size for the texture atlas.
 * @warning This function asserts that no texture patch exceeds the maximum
 *          dimensions for a texture atlas.
 */
