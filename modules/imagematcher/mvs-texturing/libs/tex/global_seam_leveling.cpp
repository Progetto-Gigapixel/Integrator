...
global_seam_leveling(UniGraph const & graph, mve::TriangleMesh::ConstPtr mesh,
    ...
    float const lambda = 0.1f; //lambda ---> hyper parameter????

    // A e Gamma rappresentano il sistema di equazioni lineari da risolvere
    ...
    //per ogni vertice i e per ogni label j (delle facce associate ai vertici adiancenti al vertice i) , per ogni vertice adjacenti k al vertice i, 
    //per ogni label l (delle facce associate ai vertici adiancenti k al vertice adjacente al vertice i)
    //prendo la label del vertice i associata alla faccia j e la label del vertice adjacenti k associata alla faccia l.
    // Se i < k e la label del vertice i e la label del vertice k sono uguali (lo stesso componente) , allora aggiungo 2 triplets alla matrice Gamma
    //come t1 e t2 (- lambda)
    //coefficiente gamma per la riga vertlabel2row[i][label] e il coefficiente gamma per la riga vertlabel2row[k][adj_vertex_label]
    for (std::size_t i = 0; i < num_vertices; ++i) {
    ...
    for (std::size_t i = 0; i < num_vertices; ++i) {
        for (std::size_t j = 0; j < labels[i].size(); ++j) {
            for (std::size_t k = 0; k < labels[i].size(); ++k) {
                ...
                if (label1 < label2) {
                    // MeshEdge -> v1 e v2 sono gli indici dei vertici che formano l'edge
                    std::vector<MeshEdge> seam_edges;
                    //dato un vertice i e due label label1 e label2, cerco le aree di seam che li collegano
    ...
    for (std::size_t channel = 0; channel < 3; ++channel) {
        ...
        cg.setMaxIterations(1000); //altri parametri da sistemare??????
    ...
    for (std::size_t i = 0; i < texture_patches->size(); ++i) {
        ...
        // Adjust values for each face of the patch
        for (std::size_t j = 0; j < faces.size(); ++j) {
