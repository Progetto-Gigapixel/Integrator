    ...
    def runphotometry(self, input_array, mask=None):
        ...
        if (mask is not None):
            ...
            for id in range(0, self.IMAGES):
                input_array[:,:,id] = np.multiply(input_array[:,:,id], mask[:,:,id]/255)
        ...
        print("Input image shape: ", h, w, self.IMAGES)
        ...
        lpinv = np.linalg.pinv(self.light_mat).T
        intensities = []
        norm = []
        for imid in range(0, self.IMAGES):
            a = input_arr_conv[imid].reshape(-1).astype(np.float32)
            #print("a: ", a)
            intensities.append(a)
            
        intensities = np.array(intensities)
        print("Light matrix shape: ", lpinv.shape)
        ...
        #norm = np.sqrt(np.sum(np.abs(rho)**2, axis=-1))
        ...
        print("Normal map shape: ", self.normalmap.shape)
        print("output_int shape: ", output_int.shape)
        ...
        if self.display:
            ...
            cv.imshow('albedo', (self.albedo * 255).astype(np.uint8))
        ...
        return self.albedo,self.normalmap
