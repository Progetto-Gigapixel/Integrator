...
def check_decimation(config, p, q):
    ...
    """
    #decimation = config.get('decimation_surface', config.get('decimation_surface', 1))
    decimation = int(config['decimation_surface'])
    if decimation <= 1:
        return p, q
    ...
    if config.get('decimation_method', '').lower() == 'coarse':
        ...
        if decimation > 1:
            ...
            p_decimated = resize(p, (int(p.shape[0] / decimation), int(p.shape[1] / decimation)), order=1, preserve_range=True, anti_aliasing=True)
            q_decimated = resize(q, (int(q.shape[0] / decimation), int(q.shape[1] / decimation)), order=1, preserve_range=True, anti_aliasing=True)
        else:
            return p, q
