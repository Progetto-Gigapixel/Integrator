...
def extract_patch(image, position, size):
    ...
    # Calcola le coordinate finali della patch
    x_end = x + width
    y_end = y + height

    # Gestisci i bordi dell'immagine
    if x < 0:
        logger.warning(f"Patch x-coordinate {x} è negativa. Impostato a 0.")
        x = 0
    if y < 0:
    ...
    if x_end > img_width:
        logger.warning(
            f"Patch x_end {x_end} supera la larghezza dell'immagine {img_width}. Impostato a {img_width}."
        )
        x_end = img_width
    if y_end > img_height:
        logger.warning(
            f"Patch y_end {y_end} supera l'altezza dell'immagine {img_height}. Impostato a {img_height}."
        )
        y_end = img_height
...
def find_files_by_format(
        path: str, file_format: str, process_subfolder: bool = False
) -> List[str]:
    """
