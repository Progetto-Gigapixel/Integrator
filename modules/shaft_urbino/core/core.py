...
class Core:
    ...
    def development_mode(self, mode=Mode.DEVELOPMENT):
        logger.info(_("Running Development Mode..."))
        ...
        for image in images:
            ...
            self.input_path = image

            # Run the development mode
            self.run(mode, correction_data)

        logger.info(_("Development Mode completed successfully."))
    ...
    def init_step(self, correction_data, step, mode):
        ...
        self.comparer = PatchComparer(
            result_file, self.reference_values, self.input_path
        )

        self.color_checker_finder = FindColorCheckerSegmentation()
    ...
    def denoising_step(
        ...
        denoising = DenoisingCorrector(
            correction_data, self.result_image, self.measured_patches, mode
        )
